...
export const Route = createFileRoute("/memoria/resultado")({
  ...
  component: ResultadoMemoria,
});

function ResultadoMemoria() {
  const search = Route.useSearch();
  const { mode, alphabet, correct, total, maxLength, avgMs } = search as {
    mode: import("@/features/memoria/types").MemoriaMode;
    alphabet: import("@/features/memoria/types").MemoriaAlphabet;
    correct: number;
    total: number;
    maxLength: number;
    avgMs: number;
  ...
  return (
    <AppShell title="Resultado" back="/memoria">
      <div className="mt-6 text-center">
        <p className="text-xs uppercase tracking-wider text-muted-foreground">
          {MEMORIA_MODE_LABEL[mode].label} · {MEMORIA_ALPHABET_LABEL[alphabet]}
        </p>
      ...
      <div className="mt-8 grid grid-cols-3 gap-2">
        ...
        <Stat label="Maior seq." value={String(maxLength)} />
