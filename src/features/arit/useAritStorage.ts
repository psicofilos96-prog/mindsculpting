...
import { DEFAULT_CONFIG, type BufferConfig, type BufferStats } from "./types";

const KEY = "neurotrainer:buffer-continuo";
...
const DEFAULT: Storage = {
  ...
  stats: { sessions: 0, totalRounds: 0, totalCorrect: 0, bestStreak: 0 },
};
...
export function useAritStorage() {
  ...
  const setConfig = useCallback((patch: Partial<BufferConfig>) => {
    setStore((prev) => {
  ...
  const recordSession = useCallback((rounds: number, correct: number, bestStreak: number) => {
    setStore((prev) => {
      const next: Storage = {
        ...prev,
        stats: {
